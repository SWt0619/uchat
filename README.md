...
一个**从零手写、不依赖任何前端框架**的自建聊天室：文字 / Markdown + 公式 / 图片 / 文件、语音与视频通话、屏幕共享、一起听歌、还有两个会聊天的 AI 用户。服务端用 Java 17 + Spring Boot，前端是原生 HTML/CSS/JS，整个部署就是**一个 jar + 一个便携 JDK**。

> 适合：想在**自己电脑/小服务器**上给几个朋友开一个私密聊天室，不依赖任何第三方服务。
