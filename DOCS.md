...
| 测试 | JUnit 5 |
| 通话容量 | 视频/语音各 2 间，每间 4 人（`chat.call.max-per-room`） |
...
| 位置 | 显示 | 说明 |
|---|---|---|
| 共享者（共享条） | `👥 3 人观看 · 上行 ≈ 4.5 Mbps（每路 1.50）` | 人数 × 每路码率（只跟帧率有关：15/30fps→1.5M、60fps→3M、≥90fps→5M）；≥85% 出口估值时转红并提示 |
...
屏幕共享 (getDisplayMedia)。480p/720p/1080p + 15/30/60/90/120fps 切换、查看器拖拽/缩放/最大化、新人主动请求观看。
...
| 房间人数 4 人 | `chat.call.max-per-room`（Mesh 物理上限），移除「≥6 人静默关视频」的隐式行为 |
...
| 屏幕共享 | 480p-1080p、15-120fps、三层防覆盖、定向信令路由 |
